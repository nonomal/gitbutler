<script lang="ts">
	...
	import { RemoteFile, Branch, commitCompare } from '$lib/vbranches/types';
	...
	async function loadRemoteBranch(name: string) {
		...
		upstream?.commits.forEach((upstreamCommit) => {
			...
			if (match) match.relatedTo = match;
		});
		const commits = upstream?.commits.filter(
			(remoteCommit) => !branch.commits.find((commit) => remoteCommit.id == commit.id)
		);
		upstreamCommits.set(commits);
	}
